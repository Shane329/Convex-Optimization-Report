...
function [P_opt, cost_opt] = solve_eld_lambda_with_losses(cost_coeff, P_limits, P_load, B, B0, B00)
    ...
    while ~converged && iter < max_iter
        % Calculate penalty factors and generator outputs
        for i = 1:NG
            ...
            % Update generator output with penalty factor
        ...
        % Calculate total losses (FIXED matrix multiplication)
        P_loss = P' * B * P + B0' * P + B00;
        
        % Check power balance
        power_mismatch = sum(P) - (P_load + P_loss);
        
        if abs(power_mismatch) < tol
            converged = true;
        else
            % Update lambda using bisection method
            if power_mismatch < 0
                lambda_min = lambda; % Need more generation
            else
                lambda_max = lambda; % Need less generation
            end
            lambda = (lambda_min + lambda_max)/2;
        end
    ...
    if ~converged
        warning('Lambda iteration did not converge within maximum iterations');
    end
end
